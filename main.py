...
def extract_episodes( id ):
    ...
    episodes = table_finds.find_all('tr', class_='change even')
    links = []
    name_episodes = []
...
def extract_subs( link ):
    ...
    subs_lists = table_select.find_all('tr', class_='change even expandable')

    info_subs = []

    #nombre - fecha_subido - nro_downloads - autor #
    for sub in subs_lists:
        name_sub = re.sub(r'\s+','',sub.find('strong').get_text())
...
def download_file(url_sub):
    ...
    file_name = '/home/brian/Descargas/subtitles/subs_downloads/' +soup_download.find('a', id='bt-dwl-bt').get('data-product-file-name')
...
if __name__=='__main__':
    ...
    for number,sub in enumerate(info_subs):
        print(f'{number}) {sub[0]} - {sub[1]} - {sub[2]}')
    selected_sub = int( input('enter the select sub: ') )

    # ---------------------------------------------------------------------#
    download_file(info_subs[selected_sub][2])
